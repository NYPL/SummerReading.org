<?php
	...
	global $isbnField;
	global $delim;
	$projField = "SR2011";		// Summer Reading Project Prefix
	$partialField = "PENDING";	// Summer Reading Pending Prefix
	$sridField = "SRID";		// Summer Reading SRID Prefix
	$dateField = "DATE";		// Summer Reading Signup Date Prefix
	$isbnField = "ISBN";		// Summer Reading ISBN Prefix
	$delim = "_-_";				// Summer Reading Field delimiter
	...
	$sr_tag = "u";				// Summer Reading field tag
	$sr_idx = "n";				// Summer Reading index tag -- Normally value is same as field tag
	$errorMsg ="";
	$status = "default";		// Checks for busy records, no record and  duplicate SRID
	$check_srid = "";			// Returned SRID
	$task_complete = 0;			// Flag to determine if task is completed
	$generate_srid = 0;			// Flag to randomly self-generate srid. DISABLE when SRID is provided by FlightPath.
...
// VERIFY	Verify PIN
    ...
    */
	...
	$delete = db_query("DELETE FROM iii_log WHERE date < DATE_SUB(NOW(), INTERVAL 10 DAY)");		// Maintain log table from growing too large
	//$num = $dbh->exec($delete);
	...
	switch ($action) {
		case "PARTIAL":		// Partial Signup
				...
				if ($patronObj != "fail" ) {
					...
					elseif ($status == "busy") {
						$task_complete = 0;	
						$errorMsg = "FAIL - Busy Record $barcode. -- Task added to queue to be proccessed later.";
		...
		case "ADD":		// OptInSummer Reader
				...
				if ($patronObj != "fail" ) {					
					...
					$insert = db_query("INSERT INTO $table ( date, log ) VALUES ( '%s', '%s' )", $date, $log);
					//$logging = $dbh->query($insert);
				}
				else {
					if ($status == "duplicate") {
						$task_complete = 1;			
						$errorMsg = "Oops! $action LIBRARY CARD FAIL - Duplicate Summer Reading ID $check_srid already in Library Patron Record $barcode.";	
					...
					elseif ($status == "busy") {
						$task_complete = 1;			
						$errorMsg = "Oops! $action LIBRARY CARD FAIL - Busy Record $barcode. -- System busy, please try again later.";
					...
					elseif ($status == "norecord") {
						$task_complete = 1;
						$errorMsg =  "Oops! $action LIBRARY CARD FAIL - Library Patron Record $barcode not found.";
						$log = "$action FAIL - Library Patron Record $barcode not found.";
						$insert = db_query("INSERT INTO $table ( date, log ) VALUES ( '%s', '%s' )", $date, $log);
						//$logging = $dbh->query($insert);	
						}
					elseif ($status == "nosrid") {
						$task_complete = 1;
						$errorMsg =  "Oops! $action LIBRARY CARD FAIL - No Summer Reading ID provided.";
						...
						$task_complete = 1;
						$errorMsg =  "Oops! $action LIBRARY CARD FAIL - Connection Error.";
						$log = "$action FAIL - Connection Error.";
						$insert = db_query("INSERT INTO $table ( date, log ) VALUES ( '%s', '%s' )", $date, $log);
						//$logging = $dbh->query($insert);	
					}
		...
		case "REMOVE":		// OptOut Summer Reader
				...
				if ($patronObj != "fail" ) {
					...
					//$errorMsg =  "Oops! $action LIBRARY CARD FAIL - Busy Record $srid. -- System busy, please try again later.";
					$log = "$action SUCCESS - Summer Reading ID $srid removed from Library Patron Record.";
					$insert = db_query("INSERT INTO $table ( date, log ) VALUES ( '%s', '%s' )", $date, $log);
					db_query("update {profile_values} set value='' where value='%s'", $srid); //remove from summerreading db
					db_query("delete from {sr_user_library_log} where luid='%s'", $srid); 
				}
				else {
					if ($status == "busy") {
						$task_complete = 1;	
					...
					elseif ($status == "norecord") {
						$task_complete = 1;
						$errorMsg =  "$action SUCCESS  - Summer Reading ID $srid not found in Library Patron Record.";
		...
		case "PRINTSR":		// Print Summer Reader
			...
			$patronObj = SearchSummerReader($srid);						// Get Patron Record with SRID
				if ($patronObj != "fail" ) {
					$task_complete = 1;
					$errorMsg =  "$date: $action SUCCESS - Print Summer Reader's Library Record $srid.";
					$log = "$action SUCCESS - Print Summer Reader's Library Record $srid.";
					$insert = "INSERT INTO $table ( date, log ) VALUES ( '$date', '$log' )";
					$logging = $dbh->query($insert);	
					//PrintPatronObj($patronObj);				
				}
...
// VERIFY	Verify PIN
	...
	while ($row=db_fetch_array($results)) {
	        $arrResult['action']= $row["action"];
			$arrResult['value1'] = $row["value1"];
			$arrResult['value2'] = $row["value2"];
	}
	foreach($arrResult as $result)
	{
			$task = $result['action'].$result['value1'].$result['value2'];
			ProcessTask($task);
			
			if ($task_complete) {
				$delete = db_query("DELETE from $table where (action = '%s') and (value1 = '%s') and (value2 = '%s')",$result['action'],$result['value1'], $result['value2']);						
			}
